...
-- Data input settings
cmd:option('-input_h5','info/im2text.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','info/im2text.json','path to the json file containing additional info and vocab')
cmd:option('-cnn_proto','model/VGG_ILSVRC_16_layers/VGG_ILSVRC_16_layers_deploy.prototxt','path to CNN prototxt file in Caffe format. Note this MUST be a VGGNet-16 right now.')
cmd:option('-cnn_model','model/VGG_ILSVRC_16_layers/VGG_ILSVRC_16_layers.caffemodel','path to CNN model file containing the weights, Caffe format. Note this MUST be a VGGNet-16 right now.')
cmd:option('-start_from', 'model/model_coco.t7', 'path to a model checkpoint to initialize model weights from. Empty = don\'t')
...
cmd:option('-batch_size',200,'what is the batch size in number of images per batch? (there will be x seq_per_img sentences)')
...
cmd:option('-optim_alpha',0.5,'alpha for adagrad/rmsprop/momentum/adam')
...
cmd:option('-cnn_learning_rate',.001,'learning rate for the CNN')
...
cmd:option('-checkpoint_path', 'model/im2text/', 'folder to save checkpoints into (empty = this folder)')
cmd:option('-language_eval', 0, 'Evaluate language as well (1 = yes, 0 = no)? BLEU/CIDEr/METEOR/ROUGE_L? requires coco-caption code from Github.')
cmd:option('-losses_log_every', 25, 'How often do we snapshot losses, for inclusion in the progress dump? (0 = disable)')

-- misc
cmd:option('-backend', 'cudnn', 'nn|cudnn')
cmd:option('-id', 'i2t_r1', 'an id identifying this run/job. used in cross-val and appended when writing progress files')
